...
class testSystem:
    #printMe = ui.CCMain.drawUI(QMainWindow).logBox.text()
    #print(printMe)
    def runSampleTest():
        for filename in os.listdir('eyes/human'):
            file = filename
            filestring = '/'.join(['eyes/human', file])
            logText = ' '.join(["Testing the following image: ", filestring])
            #ui.CCMain.drawUI().logBox.setText(logText)
...
cv2.destroyAllWindows() #terminate program
>>>>>>> c072ddbfdb8351c90c69000812ce5d38fb897f61
